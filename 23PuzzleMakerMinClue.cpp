...
int main()
{
    ...
    int OriginalPuzzle[SIZE][SIZE];
    for (int i = 0; i < SIZE; i++)
    {
        for (int j = 0; j < SIZE; j++)
        {
            Puzzle[i][j] = Starter[i][j];
            OriginalPuzzle[i][j] = Starter[i][j];
        }
    }
    ...
    do
    {
        ...
        // Pick a random cell and symbol
        int RandRow = (rand() % SIZE);
        int RandCol = (rand() % SIZE);
        int RandSymbol = (rand() % SIZE) + 1;

        // toggle for picks to be overwritten
        while (OriginalPuzzle[RandRow][RandCol] != 0)
        {
            RandRow = (rand() % SIZE);
            RandCol = (rand() % SIZE);
            RandSymbol = (rand() % SIZE) + 1;
        }

        Puzzle[RandRow][RandCol] = RandSymbol;
        OriginalPuzzle[RandRow][RandCol] = RandSymbol;

        // cout << "Trying [" << RandRow << ", " << RandCol << "] = " << RandSymbol << endl;

        SolveSudoku(Puzzle);

        // cout << "Got through Solve";

        if (NumberOfSolutions < 1)
        {
            Puzzle[RandRow][RandCol] = 0;
            OriginalPuzzle[RandRow][RandCol] = 0;
            // cout << "Nope" << endl;
            NumberOfSolutions = 0;
        }
        // This code will print the intermediate Puzzles (that have either no solutions or multiple solutions)
        // for (int i = 0; i < SIZE; i++)
        // {
        //     cout << "{ ";
        //     for (int j = 0; j < SIZE; j++)
        //     {
        //         cout << OriginalPuzzle[i][j];
        //         if (j < SIZE - 1)
        //         {
        //             cout << ", ";
        //         }
        ...
        // }
        // cout << " }" << endl << endl;
        // This ends the code that will print out the intermediate steps

    } while (NumberOfSolutions != 1);

    cout << "Finished Puzzle; trying to parse down" << endl << endl;

    // Remove redundant clues in the naive fashion. 
    for (int i = 0; i < SIZE; i++)
    {
        for (int j = 0; j < SIZE; j++)
        {
            int TempSymbol = 0;
            TempSymbol = Puzzle[i][j];
            
            int TempPuzzle[SIZE][SIZE] = {{0}};
